...
resource "aws_security_group" "rds" {
  ...
  ingress {
    ...
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.backend_tasks.id, aws_security_group.ecs_instances.id]
  }
  egress {
    from_port   = 0
    to_port     = 0
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        ...
        },
        {
          ...
          valueFrom = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${local.name_prefix}-backend-url"
        }
      ]
      logConfiguration = {
        logDriver = "awslogs"
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        {
          name      = "DATABASE_URL"
          valueFrom = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${local.name_prefix}-database-url"
        },
        {
          name      = "JWT_SECRET"
          valueFrom = aws_secretsmanager_secret.jwt_secret.arn
